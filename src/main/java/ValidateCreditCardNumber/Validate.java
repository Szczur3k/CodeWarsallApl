...
public class Validate {
    public static boolean validate(String card){
        ...
        int[] splittedCardInts = new int[splittedCard.length];

        for (int i = 0; i<splittedCard.length; i++){
            splittedCardInts[i] = Integer.parseInt(splittedCard[i]);
        }

        for (int i = splittedCard.length-2; i>=0; i=i-2){
                splittedCardInts[i] = splittedCardInts[i]*2;
            if (splittedCardInts[i]>9){
                splittedCardInts[i] = splittedCardInts[i] - 9;
            }
        }

        int sumOfAllDigitModuleTen = IntStream.of(splittedCardInts).sum() % 10;

        return sumOfAllDigitModuleTen == 0;
    }
}
